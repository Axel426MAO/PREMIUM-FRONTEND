...
import {
  createTeacher,
  SchoolApiResponse,
  type StudentCreationPayload,
} from "../services/api";
import { Class, createClass } from "@/app/admin/schools/services/api";
        ...
        <div className="space-y-3">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-xs">
            {passwordCriteria.map((c) => {
              const isMet =
                strength.criteria[c.id as keyof PasswordStrength["criteria"]];
              return (
                <li
                  key={c.id}
...
export default function StudentFormPage() {
  ...
  useEffect(() => {
    const fetchSchools = async () => {
      try {
        const schoolsData = await getSchoolsBySecretaryId(
          user!.responsible?.secretary?.id ?? 0
        );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!formData.school_id || formData.school_id <= 0) {
      toast.error("Selecione uma escola válida.");
      return;
    }
    ...
    <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Preencha os detalhes abaixo para cadastrar um novo Professor.
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Os dados de acesso serão usados para o login do Professor na
            plataforma.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Popover
                  open={openSchoolPopover}
                  onOpenChange={setOpenSchoolPopover}
                >
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      role="combobox"
                      aria-expanded={openSchoolPopover}
