...
/**
 * Busca a lista de escolas vinculadas a uma secretaria específica.
 */
export const getSchoolsBySecretaryId = async (
    secretaryId: number
): Promise<SchoolApiResponse[]> => {
    const response = await fetch(
        `http://localhost:4000/api/schools/by-secretary/${secretaryId}`
    );
    if (!response.ok) {
        throw new Error(
            "Falha ao buscar os dados das escolas da secretaria."
        );
    }
    return response.json();
};
