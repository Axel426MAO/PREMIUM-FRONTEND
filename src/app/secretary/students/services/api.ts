...
export const createStudent = async (data: StudentCreationPayload): Promise<StudentApiResponse> => {
  try {
...
export const getById = async (id: number): Promise<StudentApiResponse> => {
  try {
    const response = await fetch(`${API_URL}/student/${id}`);
    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status}`);
...
export const update = async (id: number, data: UpdateStudentPayload): Promise<void> => {
  try {
    const response = await fetch(`${API_URL}/student/${id}`, {
