...
import { Building, MapPin, UserCircle, CheckCircle, Loader2, ArrowLeft } from "lucide-react";
...
const SecretaryFormPage: FC = () => {
  ...
  // Definição das etapas do formulário com tipagem
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Redireciona para a página de listagem após 1.5 segundos
      setTimeout(() => {
  ...
  // Objeto com as variantes da animação (estados)
                  ...
                  {currentStep === 4 && (
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4 rounded-lg border border-slate-200 bg-slate-50/50 p-4">
                        <div className="font-medium text-slate-700"><strong>Secretaria:</strong> {formData.secretary.name || 'Não preenchido'}</div>
                        <div className="font-medium text-slate-700"><strong>Endereço:</strong> {`${formData.address.street}, ${formData.address.number}` || 'Não preenchido'}</div>
                        <div className="font-medium text-slate-700"><strong>Responsável:</strong> {formData.responsible.name || 'Não preenchido'} ({formData.user.email || 'N/A'})</div>
                      </div>
                      <p className="text-xs text-slate-500 text-center">Ao clicar em "Salvar Secretaria", você confirma que todas as informações estão corretas.</p>
                    </div>
                  )}
                </motion.div>
              </AnimatePresence>
