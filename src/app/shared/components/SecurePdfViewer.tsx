...
export default function SecurePdfViewer({
  ...
  useEffect(() => {
    const checkDevice = () => {
      ...
      if (isMobileDevice) {
        setScale(0.8); // ajuste automático para caber na tela
      } else {
        setScale(1.0);
      }
    };
        ...
        {numPages && (
          <footer className="absolute bottom-0 left-0 right-0 p-3 bg-white/50 backdrop-blur-sm shadow-sm flex flex-col items-center gap-2 z-30">
            ...
            <input
              ...
              className="hidden md:block w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-gray-700"
            />
          </footer>
        )}
      </div>
