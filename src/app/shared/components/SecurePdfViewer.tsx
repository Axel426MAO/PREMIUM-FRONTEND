...
export default function SecurePdfViewer({
  ...
  useEffect(() => {
    if (numPages) localStorage.setItem(`pdf_progress_${pdfUrl}`, String(pageNumber));
  ...
  const changePage = useCallback((offset: number) => {
    ...
    setPageNumber((prev) => Math.max(1, Math.min(prev + offset * pageIncrement, numPages || 1)));
  ...
  const isDoubleLayout = layout === "double" && !isMobile && numPages && numPages > 1;
  const currentLeftPage = isDoubleLayout ? pageNumber : null;
  const currentRightPage = isDoubleLayout ? (pageNumber + 1 <= numPages ? pageNumber + 1 : null) : pageNumber;
    ...
    <>
      ...
      <style jsx global>{`
        @media print {
          ...
          body::before { content: "A impressão desta página não é permitida."; color: black; font-size: 24px; text-align: center; padding: 50px; display: block; }
        }
      ...
      >
        ...
        <header className="absolute top-0 left-0 right-0 flex items-center justify-between p-3 bg-white/50 backdrop-blur-sm shadow-sm text-gray-700 z-30">
          <div className="flex-1 text-left">
              <h2 className="text-base md:text-lg font-semibold truncate" title={title}>{title}</h2>
          </div>
          <div className="hidden md:flex flex-1 items-center justify-center gap-4">
            <button onClick={() => handleZoom("out")} className={iconButtonClasses} aria-label="Diminuir zoom"><ZoomOut /></button>
            <button 
                onClick={() => setLayout(prev => prev === 'single' ? 'double' : 'single')} 
                className={iconButtonClasses} 
                title={layout === 'single' ? "Página Dupla" : "Página Única"}
                aria-label={layout === 'single' ? "Ativar página dupla" : "Ativar página única"}
            >
              {layout === 'single' ? <BookOpen /> : <Square />}
            </button>
            <button onClick={() => handleZoom("in")} className={iconButtonClasses} aria-label="Aumentar zoom"><ZoomIn /></button>
          </div>
          <div className="flex-1 text-right">
              <button onClick={onClose} className={iconButtonClasses} aria-label="Fechar Leitor"><X /></button>
          </div>
        </header>
        ...
        <main className="relative flex-grow w-full flex items-center justify-center overflow-hidden mt-14 mb-20 md:mb-24">
          <div className="absolute left-0 top-0 h-full w-1/4 cursor-pointer z-10" onClick={() => changePage(-1)}></div>
          <div className="absolute right-0 top-0 h-full w-1/4 cursor-pointer z-10" onClick={() => changePage(1)}></div>
          ...
          <div className="relative overflow-auto max-w-full max-h-full">
              ...
              >
                <div className="flex justify-center items-center gap-4" style={{ transform: `scale(${scale})`}}>
                  {isDoubleLayout ? (
                      <>
                          {currentLeftPage && <Page key={`page_${currentLeftPage}`} pageNumber={currentLeftPage} renderTextLayer={false} className="shadow-2xl" loading={<div className="p-8">Carregando...</div>} />}
                          {currentRightPage && <Page key={`page_${currentRightPage}`} pageNumber={currentRightPage} renderTextLayer={false} className="shadow-2xl" loading={<div className="p-8">Carregando...</div>} />}
                      </>
                  ) : (
                      <Page key={`page_${pageNumber}`} pageNumber={pageNumber} renderTextLayer={false} className="shadow-2xl" loading={<div className="p-8">Carregando...</div>} />
                  )}
                </div>
              </Document>
              <div
        ...
        {numPages && (
          <footer className="absolute bottom-0 left-0 right-0 p-3 bg-white/50 backdrop-blur-sm shadow-sm flex flex-col items-center gap-2 z-30">
            <div className="hidden md:flex w-full max-w-2xl items-center gap-4">
                <button onClick={() => changePage(-1)} className={iconButtonClasses} aria-label="Página anterior"><ChevronLeft /></button>
                <input
                    type="range"
                    min="1"
                    max={numPages}
                ...
                />
                <button onClick={() => changePage(1)} className={iconButtonClasses} aria-label="Próxima página"><ChevronRight /></button>
            </div>
             <p className="text-sm text-gray-600">
                Página {pageNumber}{isDoubleLayout && currentRightPage ? `-${currentRightPage}` : ''} de {numPages}
            </p>
          </footer>
        )}
      </div>
