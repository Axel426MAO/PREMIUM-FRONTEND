...
export const getStudentsBySecretaryId = async (secretaryId: number): Promise<StudentApiResponse[]> => {
    try {
        const response = await fetch(`${API_URL}/student/by-secretary/${secretaryId}`);
        if (!response.ok) {
...
export const getStudentsBySchoolId = async (schoolId: number): Promise<StudentApiResponse[]> => {
    try {
        const response = await fetch(`${API_URL}/student/by-school/${schoolId}`);
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status}`);
        }
        return response.json();
