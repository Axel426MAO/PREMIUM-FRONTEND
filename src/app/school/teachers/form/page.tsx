...
// --- API & Types ---
import { createTeacher, type StudentCreationPayload } from "../services/api";
...
export default function StudentFormPage() {
  ...
  const [formData, setFormData] = useState<StudentCreationPayload>({
    ...
    email: "",
    password: "",
    school_id: 0, // Inicia com 0, será atualizado
    turma: "",
    idade: 0,
    phone: "",
    whatsapp: "",
  ...
  // ======================= ALTERAÇÃO PRINCIPAL =======================
  // Define o school_id do formulário com base no usuário logado assim que o componente é montado.
  useEffect(() => {
    if (user?.responsible?.school?.id) {
      const userSchoolId = user.responsible.school.id;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Validação de school_id
    if (!formData.school_id || formData.school_id <= 0) {
      toast.error(
        "A escola do seu usuário não foi identificada. Tente recarregar a página."
      );
      return;
    }
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* --- ALTERAÇÃO: Campo de escola agora é apenas informativo --- */}
              <div className="space-y-2">
