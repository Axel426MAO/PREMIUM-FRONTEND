...
// --- Icons ---
import { ArrowLeft, Loader2, Eye, EyeOff, RefreshCw, XCircle, CheckCircle, PlusCircle } from "lucide-react";

// --- API & Types ---
import { createStudent, type StudentCreationPayload } from "../services/api"; // getSchools removido, getSchoolById adicionado
import { Class, createClass, getSchoolById } from "@/app/admin/schools/services/api";
import { useUserStore } from "@/app/store/userStore"; // Adicionado useUserStore

// --- UTILITÁRIOS E CONSTANTES (sem alterações) ---
...
// --- COMPONENTE DE SENHA (sem alterações) ---
...
export default function StudentFormPage() {
  ...
  const [formData, setFormData] = useState<StudentCreationPayload>({
    name: "", email: "", password: "", school_id: 0,
    turma: "", idade: 0, phone: "", whatsapp: ""
  });
  ...
  // ======================= ALTERAÇÃO PRINCIPAL =======================
  useEffect(() => {
    const schoolId = user?.responsible?.school?.id;
    if (schoolId) {
      // Define o school_id no formulário com o ID da escola do usuário logado
      setFormData(prev => ({ ...prev, school_id: schoolId }));

      // Busca as turmas da escola do usuário
      const fetchClasses = async () => {
        try {
          // Usamos a API getSchoolById para pegar os dados completos, incluindo as turmas
          const schoolData = await getSchoolById(schoolId);
          setSelectedSchoolClasses(schoolData.classes || []);
        } catch (error) {
          toast.error("Não foi possível carregar as turmas da sua escola.");
          console.error(error);
        }
      };
      fetchClasses();
    }
  }, [user]); // Roda sempre que o objeto 'user' do store for atualizado
  // ======================= FIM DA ALTERAÇÃO =======================
  ...
  const handleAddClass = async () => {
    if (!newClassName.trim()) { toast.warning("O nome da turma não pode ser vazio."); return; }
    if (!formData.school_id) { toast.error("Escola não identificada. Recarregue a página."); return; }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!formData.school_id) { toast.error("A escola do seu usuário não foi identificada."); return; }
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Input id="name" name="name" value={formData.name} onChange={handleChange} placeholder="Ex: Maria Clara" />
              </div>
              
              {/* --- ALTERAÇÃO: Campo de escola agora é informativo --- */}
              <div className="space-y-2">
                <Label>Escola Vinculada</Label>
                <div className="flex h-10 w-full items-center rounded-md border border-input bg-muted px-3 py-2 text-sm text-muted-foreground">
                    {user?.responsible?.school?.name || "Carregando escola..."}
                </div>
              </div>
            </div>
                  ...
                  {formData.school_id ? (
                    <div className="p-4 border rounded-md space-y-4 bg-background/50">
                      ...
                      <div className="flex items-center gap-2 pt-4 border-t">
                        <Input
                          placeholder="Ou crie uma nova turma"
                          value={newClassName}
                          onChange={(e) => setNewClassName(e.target.value)}
                        />
                        <Button type="button" size="icon" onClick={handleAddClass} disabled={isClassLoading}>
                          {isClassLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <PlusCircle className="h-4 w-4" />}
                        </Button>
                      </div>
                    </div>
                  ) : (
